class Player extends Phaser.Physics.Arcade.Sprite
{
	...
	netDurationCounter = 0.0;
	netCooldownCounter = 0.0;
	...
	gunMovementImpairmentDurationCounter = 0.0
	gunCooldownCounter = 0.0;
	...
	{
		// Check if the player is dashing or attacking; don't allow eight direction movement if so
		if (this.dashDurationCounter > 0.0 || this.netDurationCounter > 0.0 || this.gunMovementImpairmentDurationCounter > 0.0) {
			return;
		}
	...
	dash()
	{
		// Check that the player is attempting to move and isn't attacking
		if ((this.moveMagnitudeX != 0 || this.moveMagnitudeY != 0) && this.netDurationCounter <= 0.0)
		{
			// Check that dash is off cooldown and isn't currently being used
			if (this.dashCooldownCounter <= 0.0 && this.dashDurationCounter <= 0.0)
			{
				// Disable collision
				this.scene.player_Enemy_Collider.active = false;
	...
	handleDashCounters(delta)
	{
		...
		if (this.dashDurationCounter > 0.0)				// player is dashing
		{
			...
			if (this.dashDurationCounter <= 0) {
				...
				this.scene.player_Enemy_Collider.active = true;
	...
	netAttack(pointer)
	{
		...
		if (this.dashDurationCounter <= 0.0)
		{
			...
			if (this.netCooldownCounter <= 0.0 && this.netDurationCounter <= 0.0)
			{
				// Stop movement
	...
	handleNetCounters(delta)
	{
		...
		if (this.netDurationCounter > 0.0)		// player is attacking
		{
		...
		if (this.netCooldownCounter > 0.0)		// net is on cooldown
		{
			// Decrement counter until it's 0
	...
	gunAttack(pointer)
	{
		...
		if (this.dashDurationCounter <= 0.0)
		{
			...
			if (this.gunCooldownCounter <= 0.0)
			{
				// Stop movement
				this.body.setAcceleration(0);
				this.body.setVelocity(0, 0);
	...
	handleGunCounters(delta)
	{
		...
		if (this.gunMovementImpairmentDurationCounter > 0.0)		// player is movement impaired
		{
			// Decrement counter until it's 0
		...
		if (this.gunCooldownCounter > 0.0)							// gun is on cooldown
		{
			// Decrement counter until it's 0
